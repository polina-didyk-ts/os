...
export const POST = apiHandler(async (req) => {
  ...
  const { subject, message, recipientEmails, channel } = sendAnnouncementSchema.parse(
    await req.json()
  );
    ...
    sendEmail
      ? Promise.all(
          recipientEmails.map((to) =>
            sendAnnouncementEmail({ to, subject, message })
              .then(() => ({ to, ok: true }))
              .catch(() => ({ to, ok: false }))
          )
        )
  ...
  const skippedEmails = (slackResults as SlackResult[]).filter((r) => !r.ok).map((r) => r.email);
