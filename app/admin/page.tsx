    ...
    <main className="min-h-screen bg-transparent flex flex-col">
      ...
      <div className="flex-1 pb-28 px-4 py-5 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-2 gap-3 animate-fade-up [animation-delay:80ms]">
          <StatCard
            icon={<BadgeCheck className="w-6 h-6 text-gray-500" />}
            label="NEW"
            value={stats.new}
            loading={loading}
          />
        ...
        <div className="animate-fade-up [animation-delay:160ms]">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link
              href="/admin/requests"
              className="text-xs text-amber-600 uppercase tracking-wide font-grotesk"
            >
              ARCHIVE
            </Link>
          </div>
          ...
          <div className="flex flex-col gap-2">
            {loading ? (
              [1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="bg-white/60 backdrop-blur-sm rounded-2xl p-4 flex items-center gap-3 animate-pulse border border-white/40"
                >
